...
const oddNumbers = randomNumbers.filter(numberOdd => numberOdd % 2 != 0);
...
const numberAbove501 = crazyNumbers.reduce((accumulator, crazyNumber) => {
  if(crazyNumber < 501){
    accumulator += 1;
  }
...
const numbers = [5, 7, 3]
const numbersPotentiation = numbers.map(number => number ** 2);
...
const showGamesName = cart.reduce((accumulator, game) => {
  return `${accumulator}- ${game.name}\n`
}, '');

console.log(showGamesName);
...
const moviesIn2000 = tarantinoMovies.filter(movies2000 => {
  if(movies2000.release > 2000){
    return movies2000.name;
  }
})

console.log(moviesIn2000);
...
const showNameSeries = tvShows.map(serieName => serieName.name);
console.log(showNameSeries);
