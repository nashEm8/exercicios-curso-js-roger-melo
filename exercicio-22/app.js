...
const arrayAnimals = animals.map(item => `${item} ${item.length}`);
...
const newFriends = friends.filter(friend => friend.nearMe);
const nearFriends = newFriends.map(friend => friend.name);
...
const filteredCountries = data.filter(pais => pais.country != 'China').reduce((accumulator, regiao) => {
    return accumulator + regiao.population;
}, 0);
