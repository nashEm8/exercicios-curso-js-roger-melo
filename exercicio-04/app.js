...
const my3FavoriteTVShows = ['LOST', 'Big bang a teoria', 'Stranger Things'];

const sentece = `Minhas 3 séries favoritas são: ${my3FavoriteTVShows.join(', ')}.`.replace(', S', ' e S');
console.log(sentece);
...
const isRandomTVShowIncluded = randomTVShow.includes('Watchmen')
...
const frase = `A string que a "typeSentence" armazena tem mais de 39 caracteres? ${typeSentence.length > 39}.`;
...
const dados = `"Todos os ${falsyValues.length} itens do array "falsyValues" são falsy. Inclusive o ${falsyValues[2]}."`;
console.log(dados);
...
const agesSum = [ages[1] + ages[3]];
const fraseArray = `A soma entre o 1º e o 3º item de "ages" é menor ou igual a 92. Essa afirmação é: ${agesSum <= 92}`;
...
const isNotAString = (randomTVShow === typeof(string));
