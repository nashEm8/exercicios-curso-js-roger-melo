...
const declaration = function(num1, num2){
  return num1 * num2;
}

console.log(declaration(2,5));
...
const valor = function(value = 'Você deve passar um valor como paramêtro!'){
  console.log(value);
}

for(let i = 0; i < 7; i++){
  let qtdString = i + 1;
  console.log(`Esta é a ${qtdString} vez que essa string é exibida.`);
}
...
console.log(convertString(millennialWords));
...
const positive = function(number = 0){
  return number >= 1;
}

for(let i = 0; i < randomNumbers.length; i++){
  if(positive(randomNumbers[i])){
    numberPositive++;
  } else {
    numberNegative++;
  }
...
const getOddNumbers = function(number = []){
  for(let i = 0; i < number.length; i++){
    ...
    const numberCurrent = number[i];
    const conditionOddNumber = numberCurrent % 2 !== 0;

    if(conditionOddNumber){
      totalOddNumbers.push(numberCurrent);
    }
  }
