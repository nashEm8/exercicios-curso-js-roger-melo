...
const elementsDiv = Array.from(div.children);
...
h2.addEventListener('copy', () => {
  console.log('Texto do h2 foi copiado');
});
...
const divWithClass = document.querySelector('.egg');

divWithClass.addEventListener('mousemove', event => {
  divWithClass.textContent = `Eixo X: ${event.offsetX} | Eixo Y: ${event.offsetY}`;
});
...
button.addEventListener('click', () => {
  divWithClass.style.background = 'lightgoldenrodyellow';
});
...
/*
if(people.some(dados => dados.profession === 'Front-end developer')){
  console.log('O array people contém, no mínimo, um(a) Front-end developer.');
}
*/

const verifyProfession = people.some(({profession}) => profession === 'Front-end developer');
