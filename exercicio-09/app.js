...
const qtdCharacters = string => {
  return string.length;
}

console.log(qtdCharacters('Olá'));
...
const convertStringLowerCase = phrase => {
  return phrase.toLowerCase();
}

console.log(convertStringLowerCase('CHOCOTONE E OVO DE PÁSCOA JUNTOS NO MERCADO EM PLENO FEVEREIRO'));
...
const positionInString = (character, palavra) => palavra.indexOf(character);

console.log(positionInString('l', 'trabalho'));
/*
  05
...
let person = ['wolverine', 'magneto', 'spyke', 'Jean Grey'];

const verifyInArray = (personagem) => {
  return person.includes(personagem);
}

console.log(verifyInArray('Groxo'));
...
const unionArrays = (person1, person2) => {
  return person1.concat(person2);
}

console.log(unionArrays(person1 = ['wolverine', 'cyclops', 'psylock', 'noturno'], person2 = ['Wanda', 'Pedro', 'Lance', 'Blop']))
...
const removerLastItemArray = (xmen) => {
  xmen.pop();
  return xmen;
}

console.log(removerLastItemArray(xmen = ['wolverine', 'professor xavier', 'Fera', 'Alex']));
...
const isNull = (valor) => {
  const conditionNull = valor == null;
  if(conditionNull){
    return 'Valor é nulo!';
  } else {
    return 'Valor nãa é nulo!';
  }
}
...
const viewName = () => console.log('Emanuel Azevedo');
...
console.log(verify(33, tripleNumber));
...
numbers.forEach((number) => {
  console.log(`O ${number} do array ${numbers} é ${number}`);
})
...
const likePost = (name) => {
  switch(name.length){
    ...
    default:
      return `${name[0]}, ${name[1]} e mais ${name.length} curtiram isso`;
      break;
  }
}

console.log(likePost(['Eduardo', 'Francisco', 'Tatiana', 'Lúcia']));