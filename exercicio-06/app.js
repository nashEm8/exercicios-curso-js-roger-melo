...
const hoursNow = 21;
const conditionGoodMorning = hoursNow >= 6 && hoursNow < 12;
const conditionGoodAfternoon = hoursNow >= 12 && hoursNow < 18;

if(conditionGoodMorning){
  console.log('Bom dia!');
} else if(conditionGoodAfternoon){
...
const age = 22;
const conditionAge = age <= 7 || age >= 65;
let mensagem = '';
...
for(let i = 0; i < numbers.length; i++){
  const condition = numbers[i] >= 11 && numbers[i] <= 90;
...
for(let i = 0; i < crazyArray.length; i++){
  ...
  const isNumber = typeItem === 'number';
...
console.log(`O crazyArray tem ${qtdBool} booleans, ${qtdNumber} números e ${qtdStr} strings`);
...
for(let i = 0; i < randomNumbers.length; i++){
  const condition = randomNumbers[i] % 2 === 0;
  const itens = randomNumbers[i];
...
const everNumbers = impar.join(', ').replace(', 3', ' e 3');
const oddNumbers = pares.join(', ').replace(', 5', ' e 5');

console.log(`Números ímpares: ${everNumbers}. Números pares: ${oddNumbers}`);
