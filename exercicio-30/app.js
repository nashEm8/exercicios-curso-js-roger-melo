...
const getDiceUser = async () => {
  const response = await fetch('https://api.github.com/users/nashEm8');
  return await response.json();
}

const showDiceUser = async () => {
  const user = await getDiceUser();
  console.log(user);
}
...
const newNumbers = [];
const verifyNumbers= (number) => number % 2 === 0 || number % 3 === 0;

numbers.map(number => verifyNumbers(number) ? newNumbers.push(number) : '' );
...
const name = ['ema', 'nu', 'el'];
const Plinguage = name.map(sib => `P${sib}`);

console.log(Plinguage);
...
const lettersPosition = myName.split('');

lettersPosition.forEach((letter, index) => console.log(`${letter} é a ${index + 1} letra do meu nome`));
...
console.log(Object.keys(infors));
...
const getValueInArray = (array, value) => {
  let qtd = '';
...
console.log(getValueInArray(scores, 100));
