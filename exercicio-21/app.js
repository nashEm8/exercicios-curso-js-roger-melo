...
const orderCharacters = characters.sort((character1, character2) => character1.id - character2.id);
...
const numbersOrder = numbers.sort((number1, number2) => number1 - number2);
...
const numberLarger = randomNumbers.filter((number) => number > 50);
...
const newPeople = people.map(name => name).sort().reverse();
...
const changeIngredients = ingredients.reduce((accumulator, item, index, array) => {
  const correctWord = item[item.length - 1] === 'a' ? 'cozida' : 'cozido';

  if(index === array.length - 1){
    return accumulator + `${item} ${correctWord}`
  }
  return accumulator + `${item} ${correctWord}, `
} , '');
...
const viewMovieDisney = topBrazilmovies.filter((movie) => 
  movie.distributedBy === 'Disney').reduce((accumulator, item) => accumulator + item.peopleAmount, 0)

console.log(viewMovieDisney);
...
const dogArray = pets.filter(pet => pet.type === 'Dog').map(dog => ({
  name:dog.name, 
  age: dog.age * 7, 
  gender: dog.gender, 
  type: dog.type
...
const insertMovie = topBrazilmovies.map(movie => `<li>${movie.title}</li>`).join('');

ul.innerHTML = insertMovie;
