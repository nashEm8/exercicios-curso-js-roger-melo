...
localStorage.setItem('name', 'Emanuel');

const getItemLocalStorage = name => console.log(JSON.stringify(name))

getItemLocalStorage(localStorage.getItem('name'));
...
const combineOperations = (initValue, arrOfFunc) => 
  arrOfFunc.reduce((acc, func) => func(acc), initValue)
...
const searchAlbumExistInArray = albums.some(album => album.id === searchAlbum.id)
...
const createElement = (elementName, attributes) => {
  ...
  const atributesArray = Object.entries(attributes);

  atributesArray.forEach(([key, value]) => element.setAttribute(key, value))
